...
export async function PATCH(req: Request, { params }: { params: Promise<{ key: string }> }) {
  ...
  const existing = await prisma.customForm.findUnique({ where: { key } });
  ...
  const customForm = await prisma.customForm.update({ where: { key }, data });
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ key: string }> }) {
  ...
  const { key } = await params;
  await prisma.customForm.delete({ where: { key } }).catch(() => null);
